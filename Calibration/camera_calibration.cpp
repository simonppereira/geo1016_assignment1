...
CameraCalibration::CameraCalibration(const std::string& title, const std::string& model_file)
    : Viewer(title)
{
...
std::string CameraCalibration::usage() const {
...
bool CameraCalibration::open() {
    ...
    while (fgets(line, 256, correspondence_file) != nullptr) {
        if (5 == sscanf(line, "%f %f %f %f %f", &x, &y, &z, &xx, &yy)) {
            points_3d_.emplace_back(vec3(x, y, z));
            points_2d_.emplace_back(vec2(xx, yy));
        }
    }
...
bool CameraCalibration::mouse_drag_event(int x, int y, int dx, int dy, int button, int modifiers) {
    if (modifiers == 0)
...
bool CameraCalibration::mouse_press_event(int x, int y, int button, int modifiers) {
...
bool CameraCalibration::key_press_event(int key, int modifiers) {
    if (key == GLFW_KEY_SPACE) {
        open();
        if (points_2d_.size() < 6 || points_3d_.size() < 6) {
            std::cerr << "expecting at least 6 pairs of 3D/2D corresponding points" << std::endl;
            return false;
        }
        ...
        if (success) {
            ...
            camera_->set_from_calibration(fx, fy, skew, cx, cy, axis * angle, t);
